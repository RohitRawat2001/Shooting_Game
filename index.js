...
function animation() {
  ...
  enemies.forEach((enemy, enemyIndex) => {
    ...
    const distancebetweeenPlayerAndEnemy = Math.hypot(
      player.x - enemy.x,
      player.y - enemy.y
    );
    if (distancebetweeenPlayerAndEnemy - player.radius - enemy.radius < 1) {
    ...
    weapons.forEach((weapon, weaponIndex) => {
      const distancebetweeenWeaponAndEnemy = Math.hypot(
        weapon.x - enemy.x,
        weapon.y - enemy.y
      );
      if (distancebetweeenWeaponAndEnemy - weapon.radius - enemy.radius < 1) {
...
addEventListener("keypress", (e) => {
  if (e.key === " ") {
    ...
    // Updating Player Score in Score board in html
    scoreBoard.innerHTML = `Score : ${playerScore}`;
    hugeWeapons.push(new HugeWeapon(0, 0, "blue", HugeWeaponDamage));
  }
});
